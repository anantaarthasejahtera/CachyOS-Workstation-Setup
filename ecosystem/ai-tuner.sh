...
if ! systemctl is-active --quiet ollama >/dev/null 2>&1 && ! curl -s http://localhost:11434 >/dev/null; then
    rofi -e "❌ Ollama is not running. Please start it first!"
    exit 1
fi
...
# 3. Query Local AI
# Fast generation with temperature 0
