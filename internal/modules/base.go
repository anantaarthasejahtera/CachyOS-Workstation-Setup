...
func InstallBaseSystem() error {
	fmt.Println("🌟 [Module 01: Base] Initializing System Foundation...")
	...
	if !pacman.IsInstalled("yay") {
		...
		exec.Command("bash", "-c", installer).Run()
	}

	optimizeMakepkg()

	fmt.Println("✅ [Module 01: Base] System foundation ready.")
	return nil
}
...
func installGPUDrivers() {
	...
	if err != nil {
		fmt.Println("   Warning: Could not detect GPU.")
		return
	}

	gpu := strings.ToLower(string(out))
	fmt.Printf("   Detected: %s", string(out))
	...
	if strings.Contains(gpu, "nvidia") {
		...
		fmt.Println("⚠️  If Secure Boot is enabled, NVIDIA DKMS requires MOK key enrollment upon reboot.")

	} else if strings.Contains(gpu, "intel") {
		fmt.Println("-> Installing Intel GPU drivers...")
